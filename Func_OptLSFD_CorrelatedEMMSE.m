function [SINROptLSFDCorrelatedEMMSE, DataPowermatrix, Asave] =  Func_OptLSFD_CorrelatedEMMSE(IntDataPowerMatrix,EstError,EstPhi, CorrelatedFading, DataPowerMax, nbrBSs,K,tau,NumIter)
...
SINROptLSFDCorrelatedEMMSE = zeros(nbrBSs,K);% Space for SINRvalues;
...
SINROptLSFDCorrelatedEMMSE_Save = zeros(nbrBSs,K,NumIter);
for Iter = 1:NumIter
    ...
    for l = 1 : nbrBSs
        for k = 1:K
            ...
            SINROptLSFDCorrelatedEMMSE(l,k) = DataPowermatrix(l,k)*squeeze(Bmatrix(:,l,k))'*((Clk)\squeeze(Bmatrix(:,l,k)));
        end
    end
    SINROptLSFDCorrelatedEMMSE_Save(:,:,Iter)= SINROptLSFDCorrelatedEMMSE;
    Asave(Iter)= sum(log2(1+SINROptLSFDCorrelatedEMMSE(:)));
end
%[~,IterMax]= max(Asave);
SINROptLSFDCorrelatedEMMSE = squeeze(SINROptLSFDCorrelatedEMMSE_Save(:,:,end));
