function [SINROptLSFDCorrelatedSMMSE_Appr, DataPowermatrix, Asave] =  Func_OptLSFD_CorrelatedSMMSE_Appr(IntDataPowerMatrix,lossovernoise, EstError, InveseEstPhi, CorrelatedFading, DataPowerMax, PilotPowerMatrix,nbrBSs,K,tau, NBScases,NumIter)
%This function optimizes SINR values of all users in case of standard
%(real) MMSE estimation and approximated LSFD vectors
%
...
SINROptLSFDCorrelatedSMMSE_Appr = zeros(nbrBSs,K);% Space for SINRvalues;
...
SINROptLSFDCorrelatedSMMSE_ApprSave = zeros(nbrBSs,K,NumIter);
for Iter = 1:NumIter
    ...
    for l = 1 : nbrBSs
        for k = 1:K
            ...
            SINROptLSFDCorrelatedSMMSE_Appr(l,k) = Numerator/Denominator;
        end
    end
    SINROptLSFDCorrelatedSMMSE_ApprSave(:,:,Iter)= SINROptLSFDCorrelatedSMMSE_Appr;
    
    Asave(Iter)= sum(log2(1+SINROptLSFDCorrelatedSMMSE_Appr(:)));
end

[~,IterMax]= max(Asave);
SINROptLSFDCorrelatedSMMSE_Appr = squeeze(SINROptLSFDCorrelatedSMMSE_ApprSave(:,:,IterMax));
